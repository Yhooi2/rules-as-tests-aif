...
## §0 — Verified status snapshot (reconciled 2026-06-11)
...
| **`guard-liveness` umbrella** (staged: v0→v1→{v1.5‖v3}→v2) | change-scoped guard-liveness gate — does each manifest rule's guard actually catch its own violation? Design SSOT `2026-05-23-guard-liveness-gate.md` | 🟡 **Stage 1 DONE 2026-06-11** — v0 per-rule criticality audit (#458) + v1 change-scoped ESLint gate (#460, lazy-load fix `8dd8baa`); **Stage 2 v1.5 (cmd/script fixture gate) ‖ v3 (manual-rule pressure-scenario via SP dogfood) — UNBLOCKED, not yet dispatched**; **Stage 3 v2 full-sweep gated on v1+v1.5+v3 all merged**; umbrella `done.md` written only after v2 | #458 (v0) · #460 (v1); kickoffs `guard-liveness-v1.5-cmd-script` / `-v3-manual-sp` / `-v2-fullsweep` ready |
...
**What actually remains:** **`guard-liveness` Stage 2 (v1.5 ‖ v3, unblocked) → Stage 3 v2 full-sweep** — the one active in-flight umbrella; apply N2 verdicts (maintainer-owned rule/SSOT edits); **N7 applied + verified** — sole open item = one organic end-to-end SDD run (deferred) · **N8 A-phase C5 + cost-levers** (C1–C4 ✅ SHIPPED; cost-levers gated on §5.3 utilisation trigger) · **`meta-orch-no-arg-overview` Stage 3 UX** (Stages 0/1/2/4 ✅ — Stage 1 §0-refresh is the manual-reconciliation loop, ongoing) · **Track M.5/M.6** (M.1/M.4 ✅; mutation-discipline umbrella ✅) · `inject-layer-extension` (R-phase, deferred — DN-3 from doc-audit-ship-boundary) · N4b H10 (deferred, trigger-gated) · N5 (blocked on N7 live-dogfood). **N6b ✅ DONE** (one-click-installer, 2026-06-10).

**Open frontier (live, as of 2026-06-11 — derived from `gh` + completion-filter + dup-detect cross-check; the genuinely-open work, distinct from the ~80 historically-closed candidates the completion-filter cannot tag):**
- **`guard-liveness` Stage 2 — `v1.5-cmd-script` ‖ `v3-manual-sp`** (I-phase, S each) — **UNBLOCKED, parallel-safe, 0 open PRs**. Stage 1 (v0 #458 + v1 #460) merged 2026-06-10/11. The hottest thread; both gate Stage 3 (v2 full-sweep). Dispatch: `/pipeline guard-liveness-v1.5-cmd-script` (or `-v3-manual-sp`).
- **`guard-liveness` Stage 3 — `v2-fullsweep`** (I-phase, S) — **gated** on v1 + v1.5 + v3 all merged; umbrella `done.md` written only after this.

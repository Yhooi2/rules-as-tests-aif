...
| **`universalization-fix` umbrella** (project-philosophy audit 2026-06-14 follow-on) | residue of layout-coupling NOT closed by #511 (which closed #507/#508/#509 at the ESLint-glob+depcruise+nvmrc layer): **probe-level silent-PASS** still reports green checking zero files — R4 (`packages/core/audit-self/audit-ai-docs.ts:188` `result:'pass'` on no `src/domain`), R17 (`audit-ai-docs.react-next.sh:78` `src/shared/ui`), `test:integration` `src/**` glob → make WARN-not-PASS · S2 `check-rule-globs` into **pre-push** (earliest-reachable channel; #511 wired only validate+CI) · S3 single `resolve-source-roots` seam (R-phase: ADOPT workspace-root resolver?) replacing N hardcodes · S4 README "works as-is" layout-honesty | 🔲 **not started — startable** (deps met: #511 merged to staging, no collision on `audit-ai-docs`). Kickoff authored at `.claude/orchestrator-prompts/universalization-fix/kickoff.md` (gitignored per convention) | (none yet) — follows #511 |
...
- **`universalization-fix`** (audit follow-on, M) — layout-coupling residue NOT closed by #511: probe-level silent-PASS (R4/R17/`test:integration`) + pre-push channel gap + root-detection seam + README honesty. Kickoff authored (gitignored). **NOT blocked — startable now** (#511 merged, no collision). Source: project-philosophy audit 2026-06-14.
- **`inject-layer-extension`** (R-phase, S) — DN-3 from doc-audit-ship-boundary (#374); planned kickoff, **deferred** — startable.
- *(The three umbrellas that were the 2026-06-11 frontier all closed 2026-06-13; `universalization-fix` is the new startable frontier surfaced by the 2026-06-14 audit.)*

...
## §1.7 self-review (T15 — recursive self-application)

- **Self-application (T15):** this audit audited its own instruments — F-A/F-B name which green guards (the SHIPPED_DOCS delivery-gate, the hook unit test that fabricates its own state file and enshrines the dead state as a PASS) coexisted with the dead chain, and why (presence ≠ liveness). The corrective is the audit's own method: a full install→seed→detect probe, not a file-presence check.
- **Forward-check:** findings comply with [no-paid-llm-in-ci.md](../../../.claude/rules/no-paid-llm-in-ci.md) (all 5 probes are deterministic bash, zero API-billed calls) and read-only S1 discipline (no source fixes here; §fix-design defers to S3-C1).
- **Backward-check:** P1/P3 re-confirm the umbrella §0 pre-seeded findings against the live `./setup` path; no prior patch is superseded; the fixes are routed to the S3-C1 cluster, not applied in this read-only phase.

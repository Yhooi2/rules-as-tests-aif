...
Surface B note (verified against Claude Code hooks documentation — WebFetch of `https://code.claude.com/docs/en/hooks`, 2026-05-22): After an agent's Edit/Write tool call completes, the agent receives only the basic tool result (success/failure). IDE/LSP diagnostics (ESLint errors, TypeScript errors, markdownlint) are **not** surfaced back to the agent automatically — the only post-edit signal the harness guarantees to the agent is the explicitly-wired PostToolUse hook output. Source: «Claude Code does not automatically surface IDE/LSP diagnostics back to the agent after Edit or Write tool calls. The agent only receives: (1) tool success/failure status from the Edit/Write tool itself; (2) explicit feedback via PostToolUse hooks if configured.» Therefore a check is agent-edit-time **iff** it is wired as a surface-B PostToolUse hook. This is the framing applied throughout.

### Surface 1 — Hook-by-hook

| Hook | Current channel | Data lives in (T3 evidence) | Floor | Verdict |
|---|---|---|---|---|
| `check-doc-authority.sh` | PostToolUse `Edit\|Write` (wired) + principle 09 (pre-push/CI) | Single edited file: `check-doc-authority.sh:15` — `ABS_PATH="$(cat | jq -r '.tool_input.file_path // ""' 2>/dev/null || true)"` — reads file path from hook JSON, passes to `09-doc-authority-hierarchy.bin.ts` over single `REL_PATH` | single file content | **ALREADY-AT-FLOOR** (model dual-channel) |
| `validate-prompt.sh` | PostToolUse `Edit\|Write` (wired) + pre-push §6 (dormant guard) | Single edited file: `validate-prompt.sh:18` — `FILE_PATH="$(cat | jq -r '.tool_input.file_path // ""' 2>/dev/null || true)"` — reads file path from hook JSON; line 21 filters to `.claude/orchestrator-prompts/**/*.md` only | single file content | **ALREADY-AT-FLOOR** (edit-time is floor for single-file check; §6 pre-push is dormant backstop for force-added gitignored files) |
| `check-hook-marker.sh` | PostToolUse `Edit\|Write` (wired) | Single edited file: `check-hook-marker.sh:26` — `ABS_PATH="$(printf '%s' "$INPUT" | jq -r '.tool_input.file_path // ""' 2>/dev/null || true)"` — reads file path; line 31-33 filters to `.claude/hooks/*.sh` only | single file content | **ALREADY-AT-FLOOR** (fires at agent edit-time on hook files, which is the earliest reachable channel for hook marker presence) |
| `check-kickoff-traps.sh` | PostToolUse `Edit\|Write` (wired) | Single edited file: `check-kickoff-traps.sh:25` — `ABS_PATH="$(printf '%s' "$INPUT" | jq -r '.tool_input.file_path // ""' 2>/dev/null || true)"` — reads file path; line 38 — `CONTENT="$(cat "$ABS_PATH" 2>/dev/null || true)"` — reads full file content | single file content | **ALREADY-AT-FLOOR** (edit-time gate on kickoff.md — the only reachable channel given gitignored path; principle 12 is CI backstop) |
| `inject-matching-rule.sh` | PostToolUse `Edit\|Write` (wired) | Single edited file path: `inject-matching-rule.sh:25` — `ABS_PATH="$(printf '%s' "$INPUT" | jq -r '.tool_input.file_path // ""' 2>/dev/null || true)"` — reads file path; line 32 derives `REL_PATH`; matches against `.claude/rules/*.md` glob markers | single file path + rule files (read from disk) | **ALREADY-AT-FLOOR** (injection hook by design — not a gate; breadth is path-scoped deterministic per channel-selection §4 ADAPT) |
| `inject-session-bootstrap.sh` | UserPromptSubmit (wired) | Static digest: `inject-session-bootstrap.sh:6` — `cat <<'DIGEST'` — outputs hardcoded text block with no file reads | static text | **ALREADY-AT-FLOOR** (always-on injection per §4 catalogue; goal-invariant digest belongs at UserPromptSubmit, not edit-time) |
| `deps-hash-check.sh` | UserPromptSubmit (wired) | `.ai-factory/tool-decisions.md` + `package.json`: `deps-hash-check.sh:13` — `DECISIONS=".ai-factory/tool-decisions.md"` — reads DECISIONS file; line 19 extracts stored hash via grep; line 23 reads `package.json` to compute current hash | two files (tool-decisions + package.json) | **ALREADY-AT-FLOOR** — the check compares deps hash across the whole project on each prompt; a PostToolUse hook would only fire when package.json is edited, missing changes from other entry points. UserPromptSubmit is the correct channel for a session-start invariant check. The current channel matches the check's nature. |
| `ask-question-reminder.sh` | PreToolUse `AskUserQuestion` (wired) | Session state (loop guard via temp file, recency flag): `ask-question-reminder.sh:32-33` — `session_id=$(echo "$input" | jq -r '.session_id // "nosession"')` + `flag="${TMPDIR:-/tmp}/aif-ask-reminded-${session_id}"` — temporal recency guard with session-scoped flag file; line 38 checks `if [ -f "$flag" ]` | session state (no file data) | **ALREADY-AT-FLOOR** (PreToolUse on AskUserQuestion is the only deterministic channel for pre-question challenges; firing earlier would have no AskUserQuestion to gate) |
| `end-of-turn-reminder.sh` | Stop (wired) | Transcript path: `end-of-turn-reminder.sh:12` — `transcript=$(echo "$input" | jq -r '.transcript_path // empty' 2>/dev/null || echo "")` — reads transcript path from Stop hook JSON and parses the transcript file | transcript content | **ALREADY-AT-FLOOR** (Stop is the only channel that fires at end-of-turn; no earlier channel has transcript data) |
...
|---|---|---|---|---|---|
| §1 actionlint | pre-push + CI `actionlint` job | `workflowYmlFiles()` at `pre-push.ts:67-74`: uses `readdirSync(dir).filter(f => f.endsWith('.yml'))` — reads **ALL** `.github/workflows/*.yml` files (whole-dir scan, not a diff filter) | whole `.github/workflows/` directory | **ADD-DUAL-CHANNEL** | At edit-time: the hook has the single edited `.yml` file. It needs: only that one file (actionlint can run on a single file). Reachable? YES — `actionlint <file>` works on a single file. Surface B (agent edit-time) via a PostToolUse hook that runs `actionlint` on the edited `.yml`. Note: the pre-push backstop continues running actionlint over ALL workflow files (whole-dir); the edit-time companion covers ONLY the currently-edited file. Standard ADD-DUAL-CHANNEL pattern. Note: actionlint must be installed (pre-push already requires it). |
...
**Count reconciliation (§2 says 55, §11 says 46):** The 9-unit gap is not missing verdicts — it is Surface-4 CI-only checks that re-run the same underlying checks already classified under Surface 3 (principle tests) and Surface 5 (pre-commit sub-checks) as a whole-repo backstop. For example, the «Research patch scope annotations» CI step in audit-self.yml enforces the same invariant as principle 10 (already classified in Surface 3); «Bash syntax of ALL *.sh» CI step backstops the pre-commit bash-syntax sub-check (already classified in Surface 5). Counting these CI backstop steps as distinct verdicts in §11 would double-count; they are instead noted in Surface 4 to document the channel chain but collapse into the same verdict as their earlier-channel counterpart. The §11 total of 46 unique verdicts reflects the de-duplicated check population; the §2 total of 55 reflects the full channel-surface count including deliberate backstop redundancy.

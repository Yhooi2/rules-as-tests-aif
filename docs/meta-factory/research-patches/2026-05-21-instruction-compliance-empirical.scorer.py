...
# --- exact hook regexes (mirrors end-of-turn-reminder.sh claim-scan) ---
RE_NUM = re.compile(r'[0-9]+\+? *(files?|tests?|cases?|entries|entry|rules?|principles?|layers?|incidents?|candidates?|commits?|hooks?|lines?)', re.I)
...
def score_file(path, strict=False):
    events=[]
    try:
...
def main():
    ...
    print(f"=== claim-groundedness scorer — mode: {mode} ===")

...
N=500 дает INCONCLUSIVE-insufficient-sample по §1.5 (< 10 matches). N=750 — минимальное значение, достаточное для measurement. При N=1000 и N=1500 количество matches резко растёт (41 и 173) — длинные turns начинают включать многопарагрфные research-тексты, что ожидаемо ухудшает FP_rate. Выбор N=750 консервативен (узкий фильтр; если FP_rate < 20% при N=750, он останется < 20% при N=500).

...
| **F4 — No production-grade analog surfaces** (commitlint/semantic-release/commitizen don't fit) | SSOT BUILD entry; budget +1 day for hand-roll bash implementation. **Cost-threshold escalation to operator** (no fixed numeric threshold — agent re-review `afdc9688180da109f` flagged «3-day threshold» as arbitrary; replaced with operator escalation trigger): if hand-roll implementation reveals scope beyond «extend `.husky/pre-push` ≤80 LOC», escalate per «Cost exceeds estimate by >50%» rule in Escalation triggers below. | +1 day OR operator escalation |

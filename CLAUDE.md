...
## Satellite doctrine (two-axis — operator vs shipped)

The project relates to its dev-companions (**Superset, Superpowers, Claude Code, aif-handoff, AI Factory**, …) along **two independent axes**. A capability's verdict can — and routinely does — differ between them (e.g. SSOT [#86](docs/meta-factory/prior-art-evaluations.md): `REJECT` shipped / `ADOPT-available` operator). Never collapse the two: reading a shipped-axis `REJECT` as "the operator may not use this" is a verdict-misread (incident 2026-06-01, Superset re-survey — corrected in SSOT #86/#98/#99).

- **Operator axis — the tooling the maintainer builds *with*.** Default: **use the companions maximally; don't reinvent the wheel.** This is [build-first-reuse-default.md](.claude/rules/build-first-reuse-default.md) §1 (ADOPT/REFERENCE default) applied to the maintainer's own dev cycle. A companion feature that closes a real `/meta-orchestrator` pipeline gap is `ADOPT`-on-the-operator-axis, not merely `REFERENCE`d. No license/OS/portability constraint applies here — it's the maintainer's machine.
- **Shipped axis — the product installed into *any* consumer project.** Default: **AI-/OS-/license-agnostic core that befriends the companions and degrades gracefully when they are absent** ([dual-implementation-discipline.md §3](.claude/rules/dual-implementation-discipline.md): operator-present/consumer-absent, the runtime-bridge + aif-handoff pattern). Shipped artefacts may *integrate with* a companion but must not *hard-depend* on one (a consumer may be on Linux/Windows, OSI-only, no Superset). `install.sh --companions` (install.sh:62-69) is the existing seam for **optionally** setting a companion up in a consumer project when absent — keep it opt-in and degrading; making a companion **mandatory** for consumers is a goal change ([README.md#why-this-exists](README.md#why-this-exists)), not an operational call.

**The filter on both axes is the same — *closes a named gap*, not "because it's cool"** (the survey lens; `#adoption-shame`'s inverse). Every wired companion is maintenance surface, so each must earn its place via a concrete pipeline gap. **Priority (maintainer, 2026-06-01):** *use* companion features already in reach within the work cycle **before** building installers or speculative integrations; build-ahead-of-need is `#integration-overhead-overestimate` ([build-first-reuse-default.md §4](.claude/rules/build-first-reuse-default.md)).

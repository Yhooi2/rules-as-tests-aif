...
- `{{DISPATCH_INSTRUCTIONS}}` → per-sub-wave dispatch block: for each row in §3, name the worktree command (Mode B) or inline Agent dispatch (Mode A) per §5 Dispatch tree.

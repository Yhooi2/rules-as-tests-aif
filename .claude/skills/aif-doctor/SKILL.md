...
- **Detect (read-only):**
  ...
  - **Can't self-heal:** `docker exec … git -C <repo> ls-remote origin staging` → `Failed to connect to github.com port 443` (the §3.3 tunnel, here biting the *base* not npm — so the container cannot fetch its way current).
...
- **Fix — `refresh-aif-base.sh` (Tier 1 when `activeTaskCount=0`; reversible):** `bash ~/.claude/refresh-aif-base.sh [staging]` — gh-API real tip → **fast no-op if already current (~1s)** → else host objects (host current: `git bundle`; host also stale: `~/.claude/sync-branch-from-api.sh` FF-only first, [`feedback_sync_branch_down_via_gh_api`]) → `docker cp` → container `git branch -f` + tracking ref → verify. **Reversibility:** prints the old SHA → `docker exec aif-handoff-agent-1 git -C <repo> branch -f staging <old>`; non-destructive to task records/worktrees. **In-flight guard:** the heal entrypoint (`~/.claude/heal.sh`) applies the refresh only when `/agent/status` `activeTaskCount=0` (do NOT yank the base under a running task); otherwise it skips.
- **Always-auto wiring (SHIPPED) — «the dispatcher calls the doctor; the doctor heals»:** `dispatch.ts` runs an env-gated preflight (`runPreflight()`) **after the dedup gate, before backend-resolve** — ship-safe NO-OP when unset, non-blocking (a heal failure warns; dispatch proceeds). The operator points the dispatcher at the **doctor's** heal entrypoint (never at the helper directly): `export RUNTIME_BRIDGE_PREFLIGHT='bash ~/.claude/heal.sh'`. `heal.sh` re-checks the in-flight guard, then runs `refresh-aif-base.sh` (which no-ops in ~1s when current). So every dispatch self-heals the base first. The dispatcher only knows «call the doctor»; the doctor owns *what* healing means (extend `heal.sh` as new Tier-1 auto-heals are codified).

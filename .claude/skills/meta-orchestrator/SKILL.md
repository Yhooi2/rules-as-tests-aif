...
| Execution-build, **autonomous (aif-handoff bridge reachable)** | Autonomous dispatch via runtime-bridge | `tsx packages/runtime-bridge/src/cli/dispatch.ts <kickoff>` when `RUNTIME_BRIDGE_MODE` ∈ {`aif-handoff`,`auto`} + aif answers on `RUNTIME_BRIDGE_AIF_URL` (#312/#313); dispatch prints the task URL. **Form:** pass a SINGLE buildable sub-wave kickoff, NOT the `-meta-launch` orchestration wrapper — aif treats the kickoff as one impl spec; a meta-plan yields ZERO code (live-confirmed a4bdff98: $5.66, no code; `dispatch.ts` now warns). **Precondition:** kickoff MUST carry §4c park-don't-guess. `await.ts` reconciles aif's optimistic `done`→BLOCKED on a no-code halt. Unreachable → `ManualBackend` fallback. See `#autonomous-dispatch-without-park` + `#dispatch-meta-launch-to-aif`. |

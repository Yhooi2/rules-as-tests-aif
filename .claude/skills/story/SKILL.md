...
> **Authoritative for:** session recap narration skill — localized story-format generation, act-by-act narrative structure, `## 🎬` marker convention, and the shared SSOT link to the Stop-hook `aif_msg_eot_branch_story` branch.
> **NOT authoritative for:** project goal — see [README.md#why-this-exists](../../README.md#why-this-exists). Output language gate — see `AIF_HOOK_LANG` convention and [.claude/hooks/inject-session-bootstrap.sh](./../hooks/inject-session-bootstrap.sh).

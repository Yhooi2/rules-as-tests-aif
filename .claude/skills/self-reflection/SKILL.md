...
- **Citations are existence-checked (L2 block).** ≥1 `file.ext:line` per section must resolve to a real file with the line in range at the PR head — a fabricated `foo.ts:42` that does not resolve **fails** the gate ([`s17-pr-body.ts`](../../../packages/core/hooks/checks/s17-pr-body.ts)). Genuine-looking citations (containing a `/`) that don't resolve, or point to a blank/brace-only line, surface as `::warning::` (L3, calibration — non-fatal). So the cheapest path to green is citing a line you actually opened.

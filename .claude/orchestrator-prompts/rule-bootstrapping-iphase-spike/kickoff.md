...
1. **`RuleResearchClient` port + deterministic stub (research/menu half).** New `packages/core/synthesizer/rule-research-port.ts` mirroring `research-port.ts`'s `ResearchClient` (T16 — same injectable live/stub seam): interface `research(detection) → ResearchPlan` (the `{framework, version, patterns[]}` menu that `synthesizeGenerate` consumes as its `plan` arg) + `stubRuleResearch` returning a FIXED react-next `ResearchPlan` with ONE pattern (a real, well-known react-next practice/anti-pattern — e.g. "no raw `<img>`; use `next/image`"). Document the port as the plug-in seam where the LIVE MCP adapter slots in next (OUT of this spike).
2. **Rule-body stub + seam to the factory (generate half).** `synthesizeGenerate(plan, client)` (`generate.ts:20`) takes TWO inputs: the step-1 `ResearchPlan` AND a `GenerateClient` that authors the rule body → `GenerateSelection` (`generate-port.ts:45-51`). Reuse the existing stub `GenerateClient` pattern (mirror `stubGenerateRN`, `generate-stubs.ts:18`): a FIXED `GenerateSelection` with ONE `GenerateCandidate` whose `entryId` MATCHES the step-1 pattern `id` (else `synthesizeGenerate` silently drops it, `generate.ts:47`), carrying `eslintConfig` (or `presence:'forbid'`+`selector`), `examples{bad,good}`, and `negativeTest`. Wire both through the EXISTING `runGeneratePath(plan, client)` (`generate-cli.ts:22`). The factory is called UNCHANGED — this is the #798 §4 seam: the live/stub part supplies only data conforming to `GenerateCandidate`, never TS. A real `findings → GenerateSelection` transform is the LIVE adapter's job (OUT of this spike); the stub returns a fixed selection.
3. **Revive the lock — do NOT double-emit.** Branch on `runGeneratePath`'s result: on `mode:'synthesis'`, pass the returned `SynthesisPlan` STRAIGHT to `install(plan, {consumerRoot, force:true})` (`install.ts:88`). `install()` ALREADY does emit + `buildLock` + re-validate internally (`install.ts:134-150`) — do NOT call `emit()` first (a pre-emit trips the `lock-collision` guard, `install.ts:108-122`); `force:true` (or a pre-cleaned output dir) keeps the fixture re-runnable. This is the first time the dead `buildLock` runs in a real flow → a real `rules-lock.json` is produced.
...
5. **Tests (paired-negative, $0):** a good stub finding → a rule+test that FIRES (non-vacuous: real `Linter.verify` violation + clean case); a bad/empty stub finding → L4 reject / no rule emitted (reuse the `stubGenerateBad` pattern, `generate-stubs.ts:195`). Mirror `generate.test.ts`. **Before Done, run the L4/L5 snapshot + install tests on the non-`--full` path and confirm ZERO diff** — proves the opted-out path stayed byte-identical (no silent snapshot regression).

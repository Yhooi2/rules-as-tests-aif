 ...
 *
 * MCP server endpoint: default http://localhost:3000/mcp (or configured via env)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 ...
 * getStatus() / awaitDone(): MVP placeholder returning 'pending' — SW-C implements
 *   real WebSocket status readback.
 *
...
export interface AifHandoffConfig {
  /**
   * Base URL of the aif-handoff MCP server.
...
export class AifHandoffBackend implements RuntimeBackend {
  ...
  getStatus(handle: TaskHandle): Promise<TaskStatus> {
    // MVP placeholder — SW-C implements real WebSocket status readback.
    // (kickoff §3 SW-B item 3: «getStatus / awaitDone: MVP returns pending placeholder»)
    void handle;
    return Promise.resolve({
      status: 'pending',
      rawStatus: 'sw-c-placeholder',
      checkedAt: new Date().toISOString(),
    });
  }

  awaitDone(handle: TaskHandle, _timeoutMs?: number): Promise<TaskResult> {
    // MVP placeholder — SW-C implements real WebSocket await.
    void handle;
    return Promise.resolve({
      success: false,
      content: '',
      finalStatus: 'sw-c-placeholder',
      completedAt: new Date().toISOString(),
      meta: { note: 'SW-C WebSocket readback not yet implemented' },
    });
  }

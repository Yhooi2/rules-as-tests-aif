#!/usr/bin/env bash
# setup.d/60-ci.sh — §6b .nvmrc drift WARN + §6b-bis R2 auto-wire Layer 1 + §6c CI-orphan WARN+wire.
# S0 rows:
#   §6b .nvmrc drift WARN → L1125-1148 (original install.sh)
#   §6b-bis R2 auto-wire Layer 1 → L1150-1209
#   §6c CI-orphan WARN + opt-in auto-wire → L1211-1356
#
# Sets _r2_verdict (global) — read by install.sh dispatcher for Layer 2 AST-wire.
#
# Variables consumed from dispatcher scope:
#   PKG_ROOT, PROJECT_ROOT, FORCE, DRY_RUN, STACK, WIRE_CI, SKIPPED (array)
# Variables exported to finalize tail:
#   _r2_verdict — set by §6b-bis Layer 1 for §6b-bis-L2 in install.sh

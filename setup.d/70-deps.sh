#!/usr/bin/env bash
# setup.d/70-deps.sh — §7 package.json scripts + §8 dev-dep install + §8b tsx-at-root.
# S0 rows:
#   §7 package.json scripts → L1358-1440 (original install.sh)
#   §8 dev-dependency install → L1442-1538
#   §8b tsx-at-root probe → L1540-1595
#
# Sets DEPS_INSTALLED (global) — read by install.sh dispatcher's finalize tail.
#
# Variables consumed from dispatcher scope:
#   PKG_ROOT, PROJECT_ROOT, FORCE, DRY_RUN, FULL, STACK, SKIPPED (array)
# Variables exported to finalize tail:
#   DEPS_INSTALLED — set to "1" when §8 dep install succeeded
#   DEVDEPS — the full dev-dep array (finalize tail uses it for the manual fallback echo)
...
if [ -f "$PROJECT_ROOT/package.json" ]; then
  if [ -n "$DRY_RUN" ]; then
    ...
    # PARK[S2+]: test:integration value contains bare single-quotes inside node -e '...' — latent
    # shell quoting bug, preserved verbatim (byte-identical invariant). Do not escape them here.

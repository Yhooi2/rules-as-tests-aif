#!/usr/bin/env bash
# setup.d/lib.sh — Shared helpers for the install.sh modular pipeline.
#
# Sourced by the thin install.sh dispatcher BEFORE any numbered layer.
# Also sourceable standalone with INSTALL_SH_LIB_ONLY=1 to unit-test helpers
# without running the full install pipeline.
#
# Public API (consumed by layers):
#   UPSTREAM_BLOB_URL          — GitHub blob base URL for internal-ref rewriting
#   transform_internal_refs()  — rewrite repo-internal .md links to blob URLs
#   copy_safe()                — copy src→dst, skip-if-exists (respects FORCE/DRY_RUN)
#   refresh_safe()             — copy src→dst, overwrite unless .override.md present
#   PRETTIERIGNORE_BEGIN / PRETTIERIGNORE_END
#   merge_prettierignore()     — non-destructive .prettierignore merge
#   PRETTIERIGNORE_CFG_BEGIN / PRETTIERIGNORE_CFG_END
#   _prettierignore_in_skipped() — test if a path is in SKIPPED
#   ignore_shipped_configs()   — add freshly-shipped configs to .prettierignore
#   mkdir_safe()               — mkdir -p respecting --dry-run
#   chmod_safe()               — chmod respecting --dry-run
#   detect_pm()                — detect consumer package manager (npm|pnpm|yarn)
#   patch_stryker_package_manager() — patch stryker.config.json packageManager key
#   copy_skill_with_transform() — copy + internal-ref transform for orchestration skills
#   refresh_skill_with_transform() — refresh + internal-ref transform for orchestration skills
#
# Variables consumed from dispatcher scope (must be hoisted by dispatcher before sourcing):
#   PKG_ROOT, PROJECT_ROOT, FORCE, DRY_RUN, SKIPPED (array)
#
# O1 fix: the lib-only guard is placed AFTER all helper definitions so that
# INSTALL_SH_LIB_ONLY=1 exposes all helpers to test harnesses.
# Model: setup.d/engine.sh:44 (guard at END).
...
# ─── Lib-only guard (O1 fix: placed AFTER all helper definitions) ────────────
# When INSTALL_SH_LIB_ONLY=1, source this file to expose all helpers above
# without running the install pipeline. Used by tests/install-sh/*.test.sh.
# Model: setup.d/engine.sh:44 (guard at END, not at start).

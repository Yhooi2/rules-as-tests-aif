#!/usr/bin/env bash
# setup.d/40-configs.sh — §4 Scripts + §5a shared configs + §5b' ESLint barrel + §6a stack configs.
# S0 rows:
#   §4 Scripts → L866-914 (original install.sh)
#   §5a shared configs (nvmrc, lintstagedrc, prettierignore, prettierrc, tsconfig, AGENTS.md) → L916-949
#   §5b' Custom ESLint barrel → L996-1060
#   §6a stack-specific templates (eslint/vitest/playwright/arch/stryker/ci) → L1062-1123
#
# Note: §5c .husky + TS-core hooks + git activation goes to 50-hooks.sh.
#
# Variables consumed from dispatcher scope:
#   PKG_ROOT, PROJECT_ROOT, FORCE, DRY_RUN, STACK, SKIPPED (array)
...
# ─── 5a. Shared templates → project root ────────────────
